...
CSV Amazon Reviews
     ...
     ▼
┌─────────────────┐     ┌───────────────┐     ┌──────────────────────────┐
│  Kafka Producer │────►│ Apache Kafka  │────►│ Spark Structured         │
│  producer.py    │     │amazon-reviews │     │ Streaming + MLlib Model   │
│  0.5s / review  │     │               │     │ (Tokenizer→TF-IDF→LR)    │
└─────────────────┘     └───────────────┘     └────────────┬─────────────┘
                                                            │
                              ┌─────────────────────────────┤ FORK
                              │                             │
                 ⚡ CHEMIN 1 LIVE (1s)        🗄 CHEMIN 2 STATS (10s)
                              │                             │
...
├── django_app/
│   ├── reviews_app/             # App Django principale
...
│   ├── static/js/               # React SPA (JSX compilé browser-side)
...
# 4. Lancer tout avec le script de démarrage
./start.sh
`
...
### Étape 1 — Notebooks ML (offline)
Les 4 notebooks Jupyter entraînent le modèle :
1. **EDA** — Exploration, distribution des sentiments, longueurs de texte
2. **Preprocessing** — Nettoyage HTML, lemmatisation NLTK, suppression stop-words (sauf négations)
3. **Training** — Pipeline Spark MLlib, CrossValidation 3-fold, sauvegarde est_model/
4. **Evaluation** — F1-macro = **0.8320**, matrice de confusion
...
### Chemin 1 — Visualisation en continu
- Spark écrit dans le topic Kafka predictions-live toutes les **1 seconde**
- Un thread Django (pps.py ready()) consomme ce topic
- Il broadcaste via channel_layer.group_send() → WebSocket → navigateur
- **Latence bout-en-bout : ~1-2 secondes**

### Chemin 2 — Analyses Statistiques
- Spark écrit dans MongoDB via oreachBatch toutes les **10 secondes**
- Le Dashboard React fait un etch() toutes les **15 secondes**
- Django exécute des aggregation pipelines MongoDB ($group, $sort)
- Chart.js redessine les graphes (KPI, donut, trend annuel, table)
...
| WebSocket /ws/live/ | Flux temps réel des prédictions |
...
> *Les fichiers > 100 MB ne sont pas versionnés. Télécharger le dataset original depuis [Kaggle — Amazon Fine Food Reviews](https://www.kaggle.com/datasets/snap/amazon-fine-food-reviews).
...
*Stack : Spark 3.5 · Kafka 7.5 · MongoDB 7 · Django 4.2 · React 18.3 · Docker Compose*
